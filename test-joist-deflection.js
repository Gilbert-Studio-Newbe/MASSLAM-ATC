// Test script for deflection limits in joist calculations
...
const deflectionLimits = [200, 250, 300, 350, 400, 450, 500];
...
// Run the tests
console.log('Defl Limit | Width (mm) | Depth (mm) | Bending (mm) | Deflection (mm) | Adjusted (mm) | Max Defl (mm)');
console.log('----------|------------|------------|--------------|----------------|---------------|------------');

deflectionLimits.forEach(deflectionLimit => {
  const result = calculateJoistSize(span, spacing, load, timberGrade, fireRating, deflectionLimit, safetyFactor);
  console.log(`     L/${deflectionLimit.toString().padStart(3)} | ${result.width.toString().padStart(10)} | ${result.depth.toString().padStart(9)} | ${result.bendingDepth.toString().padStart(11)} | ${result.deflectionDepth.toString().padStart(14)} | ${result.fireAdjustedDepth.toString().padStart(12)} | ${result.allowableDeflection.toFixed(1).padStart(11)}`);
}); 