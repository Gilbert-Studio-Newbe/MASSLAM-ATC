// Manual debugging of the joist calculation for a 9m span with residential loads
console.log("DEBUGGING JOIST CALCULATION FOR 9m SPAN WITH RESIDENTIAL LOADS");

// Constants
const span = 9.0; // meters
const spacing = 800; // mm
const load = 2.0; // kPa residential load
const fireRating = '90/90/90';  // less stringent fire rating
...
console.log(`- Load: ${load} kPa (Residential)`);
...
// Step 4: Check deflection with residential load criteria
const deflectionLimit = 300; // Residential load (L/300)
...
console.log(`\nStep 4: Check deflection with residential load criteria`);
console.log(`- Deflection limit: L/${deflectionLimit} (Residential)`);
...
const fireAllowance = 20; // mm per exposed face for 90/90/90
